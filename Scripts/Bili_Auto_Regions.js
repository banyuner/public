...
****************************
[Script]
Bili Region = type=http-response,pattern=^https:\/\/ap(p|i)\.bili(bili|api)\.(com|net)\/(pgc\/view\/v\d\/app\/season|x\/offline\/version)\?,requires-body=1,max-size=0,script-path=https://raw.githubusercontent.com/banyuner/public/master/Scripts/Bili_Auto_Regions.js
...
async function SwitchRegion(title, url, body) {
	const Group = $.read('BiliArea_Policy') || '📺 𝑩𝒊𝒍𝒊𝑩𝒊𝒍𝒊'; //Your blibli policy group name.
	const CN = $.read('BiliArea_CN') || '🎯 𝑫𝒊𝒓𝒆𝒄𝒕'; //Your China sub-policy name.
	const TW = $.read('BiliArea_TW') || '🇨🇳 𝑻𝒂𝒊𝒘𝒂𝒏'; //Your Taiwan sub-policy name.
	const HK = $.read('BiliArea_HK') || '🇭🇰 𝑯𝒐𝒏𝒈𝑲𝒐𝒏𝒈'; //Your HongKong sub-policy name.
	const DF = $.read('BiliArea_DF') || '🎯 𝑫𝒊𝒓𝒆𝒄𝒕'; //Sub-policy name used after region is blocked(e.g. url 404)
